...
export default async function RVImpaintingOrderQueue() {
	// Check if the service is running
	if ((global as any).isRVImpaintingRunning) {
	...
	console.log('///////////////////////////////');
	(global as any).isRVImpaintingRunning = true;
	...
	const processOrders = async () => {
		try {
			...
			if (orders.length === 0) {
				...
				console.log('///////////////////////////////');
				console.log('///////////////////////////////');
				(global as any).isRVImpaintingRunning = false;
				return;
			}
